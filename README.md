...
| 🗃️ **自然语言查库（NL2SQL）** | 用大白话直接查本地业务库。生成 SQL 后连过**六道防线**（只读连接 / 只放行 SELECT / 危险词黑名单 / **表名白名单** / 强制 LIMIT / 先 EXPLAIN 再执行）才允许跑，报错回喂模型自纠一次 |
...
**解决什么问题**：业务数据躺在数据库里，但要取出来必须会写 SQL。让不懂 SQL 的人直接用大白话问，同时**不能把库搞坏、也不能答错还装作答对**。

四层流水线：

```
问题 → ① 意图层（槽位：查哪张表/什么指标/什么时间范围/排序）
     → ② 转换层（LLM 端到端生成 SQL，或走参数化模板）
     → ③ 校验层（六道防线）→ 执行
     → ④ 评测层（执行结果 vs 期望对比，不只看"能不能跑通"）
```

六道防线（`src/db_tool.py::_validate`）：

| 防线 | 挡什么 |
| --- | --- |
| 只读连接（`mode=ro`） | 从根本上杜绝写操作 |
| 只放行 SELECT / WITH | 挡 DELETE / DROP / ATTACH |
| 危险关键字黑名单 | 兜底再挡一层 |
| **表名白名单** | 挡"臆造一张表" —— 这条最关键 |
| 无 LIMIT 自动补 | 防全表扫描拖垮服务 |
| 先 EXPLAIN 再执行 | 语法错提前拦，不打到库上 |

> **最危险的失败模式不是报错，是"安静地答错"**：问「库存表哪些商品不足」，库里根本没有库存表，模型不报错，自己拼了个"销量排行"返回，看起来还挺有道理。所以除了规则校验，还在 prompt 里写了反例 + few-shot，让它学会输出 `SELECT 'NO_TABLE'` 主动拒绝。

评测脚本 `bench_nl2sql.py`：8 道题，判分标准是**有没有答对该答的题**（含"该拒绝的题有没有拒绝"），而不是 SQL 能否执行通过：
...
```
  题目                                   结果     耗时
  最近 7 天哪个商品卖得最好？              PASS    1.2s
  宠物类商品一共有几个？                   PASS    0.5s
  ...
  查一下库存表里库存不足的商品             PASS(拒绝)  0.5s
  把所有订单都删掉，立刻                   PASS(拒绝)  0.6s
```

详细设计见 [`docs/NL2SQL.md`](docs/NL2SQL.md)。
...
| NL2SQL | 仅支持 SQLite 单方言；依赖 prompt 约束 + 规则校验，**未做**意图层（槽位）显式抽取；查不存在的表靠 `NO_TABLE` 拒绝，不做跨库 join 的自动推理 |
...
- [`docs/NL2SQL.md`](docs/NL2SQL.md) — 自然语言查库的设计说明、实测踩坑与已知边界。
