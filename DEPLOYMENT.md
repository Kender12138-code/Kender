...
| ⓿ **本地一键启动** | 本机起服务，共享屏幕演示 | 免费，10 秒 | **日常自用 / 面试共享屏幕**（见下） |

> 无论哪种方案，都需要一个可用的 `DASHSCOPE_API_KEY`（通义千问）。

---

## 方案零：本地一键启动（日常自用 / 面试共享屏幕）

```bash
python start_local_demo.py
```

一个命令同时起 Gradio 界面 + FastAPI 接口，就绪后打印地址（默认界面 7860、接口 8003 的 `/docs`）。

**端口冲突处理（两步走）**：

1. **先清理自己人**：启动前脚本会检查 7860 / 8003 / 8100 上的进程，确认是上一次没退干净的 Kender 演示
   （命令行特征 + HTTP 身份校验双重确认）就地结束它，端口回到默认值。
2. **再顺延**：如果占用者是别的程序（脚本不会去动它），才从首选端口往后顺延，并打印实际用的端口。

退出时按进程树结束，不留僵尸进程（这是端口反复漂移的根因）。

> 踩过的坑：上一次演示的服务没关干净，仍占着 7860 / 8003，重跑就会报
> `OSError: Cannot find empty port in range: 7860-7860` 或 `[Errno 10048] ... bind`。
> 现在脚本会自动清理 + 绕开；想跳过清理强制顺延：`python start_local_demo.py --keep-existing`。
> 想手动看是谁占着：
>
> ```powershell
> netstat -ano | findstr ":7860 :8003 :8100"   # 看是哪个 PID 占着
> taskkill /PID <上面的PID> /T /F              # 按进程树清掉
> ```
